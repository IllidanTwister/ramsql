...
func (p *parser) parseValue() (*Decl, error) {
	...
	valueDecl, err := p.consumeToken(StringToken, NumberToken, DateToken, NowToken, TrueToken, FalseToken)
	if err != nil {
...
func (p *parser) parseListElement() (*Decl, error) {
	...
	var valueDecl *Decl
	valueDecl, err := p.consumeToken(StringToken, NumberToken, NullToken, DateToken, NowToken, FalseToken, TrueToken)
	if err != nil {
		return nil, err
	}
